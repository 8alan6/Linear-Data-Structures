...
public abstract class Room  implements Serializable{
	...
	public void addGuest(Guest guest) throws MyException {
		if(getGuest().size()<getMaxGuests()) {
			...
			throw new MyException("Can't add guest as the room is full, Room number is "+this.getRoomNumber());
	}
	
	public double Price(Guest guest, Room room, int RoomNum) {
		...
		System.out.println("\t\t\tDISCOUNT  : €"+(totalPrice - fullPrice)+"0");
		System.out.println("**********************************************");
		System.out.println("\t\t\tAMOUNT DUE: €"+fullPrice+"0");
		return fullPrice;
	}
