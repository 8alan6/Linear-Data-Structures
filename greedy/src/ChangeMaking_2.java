...
public class ChangeMaking_2 {
	...
	public int selectionFunction(int changeGenerated, 
		...
		//We use 'maxValue' to keep track of the biggest value found so far. 
		//We use 'index' to compute just once the length of MyList 'items'.
		int maxValue = Integer.MIN_VALUE;
		int index = coinValues.length() - 1;
		
		//OP1. We traverse all items of MyList, to find the 'index' of the item with maximum value
		while (index >= 0){
			...
			if ((discarded.getElement(index) == 0) && (e0 > maxValue)){
				res = index;
				maxValue = e0;
			}
